...
class Monthly(Appointment):
    ...
    def occursOn(self, year, month, day):
        return (date(year=year, month=month, day=day) >= self._date) & (day == self._date.day)

    # Repr method for the OneTime class
    # returns the type of date, description and date. Eg: Monthly,Appointment,2023,7,11
    def __repr__(self):
        year = str(self.getDate().year)
        month = str(self.getDate().month)
        day = str(self.getDate().day)
        return "Monthly,"+self.getDescription()+","+year+","+month+","+day
